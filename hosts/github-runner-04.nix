...
# Hetzner dedicated GitHub Actions runner for fuww org (github-runner-04)
# EX63 dedicated server (Intel Core Ultra 7 265, 64GB RAM DDR5 ECC, 2x 1TB NVMe).
# IP: 178.63.233.19 / 2a01:4f8:2240:1219::2
#
# Provisioning (rescue mode ships the joost+agent@fashionunited.com key
# registered at order time, so run from a machine holding that private key):
#   1. Confirm server is reachable: ssh root@178.63.233.19
#   2. Wipe any leftover RAID superblocks + confirm UEFI (in rescue):
#        for d in /dev/nvme0n1 /dev/nvme1n1; do mdadm --stop --scan; \
#          mdadm --zero-superblock --force "$d" 2>/dev/null || true; wipefs -af "$d"; done
#        [ -d /sys/firmware/efi ] && echo UEFI || echo BIOS
#   3. Run: make hetzner/provision NIXADDR=178.63.233.19 NIXNAME=github-runner-04
#   4. SOPS re-key (host age key changes from loom's placeholder to real):
#        ssh-keyscan 178.63.233.19 | grep ed25519 | ssh-to-age
#        # Replace the github-runner-04 anchor in .sops.yaml with the new key
#        sops updatekeys secrets/github-runner-04.yaml
#        # Rebuild so the runner can decrypt the token
#        make hetzner/copy NIXADDR=178.63.233.19 NIXUSER=joost
#        make hetzner/switch NIXADDR=178.63.233.19 NIXNAME=github-runner-04 NIXUSER=joost
#   5. Set up Tailscale: make hetzner/tailscale-auth NIXADDR=178.63.233.19 TAILSCALE_AUTHKEY=tskey-auth-xxx
#   6. Rotate the github-runner-token in SOPS with a fresh org registration token:
#        sops secrets/github-runner-04.yaml
...

{
  ...
  # Hostname
  networking.hostName = "github-runner-04";
  ...
  services.nixosAutoUpdate = {
    enable = true;
    flake = "github:javdl/nixos-config#github-runner-04";
    dates = "04:00";
  ...
  sops.defaultSopsFile = ../secrets/github-runner-04.yaml;
    ...
    lib.listToAttrs (
      lib.genList (
        ...
        lib.nameValuePair "fuww-runner-${toString idx}" (
          common
          // {
            name = "github-runner-04-${toString idx}";
  ...
  # GATED OFF: this box was unreachable (Tailscale + public 22 both timed out) on
  # 2026-07-28, so its javdl token couldn't be inserted. Flip to true after
  # putting github-runner-javdl-token in secrets/github-runner-04.yaml on the host.
  services.javdlRunners = {
    enable = false;
    namePrefix = "github-runner-04-javdl";
    tokenSopsFile = ../secrets/github-runner-04.yaml;
  };
