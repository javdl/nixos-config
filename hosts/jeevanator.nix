...
#   2. Run: make hetzner/bootstrap0 NIXADDR=<ip> NIXNAME=jeevanator
#   3. After reboot: make hetzner/bootstrap NIXADDR=<ip> NIXNAME=jeevanator
#   4. Connect: ssh jeevanator (uses ~/.ssh config)

{
  imports = [
    ./hardware/jeevanator.nix
    ../modules/cachix.nix
    ../modules/secrets.nix
  ...
  # Hostname
  networking.hostName = "jeevanator";
  ...
  services.nixosAutoUpdate = {
    enable = true;
    flake = "github:javdl/nixos-config#jeevanator";
    dates = "04:00";
  ...
  users.users.jeevan.shell = lib.mkForce pkgs.zsh;
