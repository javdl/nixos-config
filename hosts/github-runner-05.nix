...
# Hetzner dedicated GitHub Actions runner for fuww org (github-runner-05)
# EX63 dedicated server (Intel Core Ultra 7 265, 64GB RAM DDR5 ECC, 2x 1TB NVMe).
# IP: 144.76.86.24 / 2a01:4f8:192:12cf::2
#
...
#   1. Confirm server is reachable: ssh root@144.76.86.24
...
#        make hetzner/copy NIXADDR=144.76.86.24 NIXUSER=joost
#        make hetzner/switch NIXADDR=144.76.86.24 NIXNAME=github-runner-05 NIXUSER=joost
#   5. Set up Tailscale: make hetzner/tailscale-auth NIXADDR=144.76.86.24 TAILSCALE_AUTHKEY=tskey-auth-xxx
#   6. Rotate the github-runner-token in SOPS with a fresh org registration token:
#        sops secrets/github-runner-05.yaml
...

{
  ...
  # Hostname
  networking.hostName = "github-runner-05";
  ...
  services.nixosAutoUpdate = {
    enable = true;
    flake = "github:javdl/nixos-config#github-runner-05";
    dates = "04:00";
  ...
  sops.defaultSopsFile = ../secrets/github-runner-05.yaml;
    ...
    lib.listToAttrs (
      lib.genList (
        ...
        lib.nameValuePair "fuww-runner-${toString idx}" (
          common
          // {
            name = "github-runner-05-${toString idx}";
