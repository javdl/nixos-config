...
# Hetzner dedicated server for lennard (lennardroid)
# IP: 46.225.182.179 (Nuremberg, CPX32) #121364784
# IPv6: 2a01:4f8:1c19:6511::/64
#
# Bootstrap process:
#   1. Boot Hetzner server into rescue mode (Linux 64-bit)
#   2. Run: make hetzner/bootstrap0 NIXADDR=46.225.182.179 NIXNAME=lennardroid
#   3. After reboot: make hetzner/bootstrap NIXADDR=46.225.182.179 NIXNAME=lennardroid
#   4. Connect: ssh lennardroid (uses ~/.ssh config)

{
  imports = [
    ./hardware/lennardroid.nix
    ../modules/cachix.nix
    ../modules/secrets.nix
  ...
  # Hostname
  networking.hostName = "lennardroid";
  ...
  services.nixosAutoUpdate = {
    enable = true;
    flake = "github:javdl/nixos-config#lennardroid";
    dates = "04:00";
  ...
  users.users.lennard.shell = lib.mkForce pkgs.zsh;
