...
# Hetzner dedicated server for peter (peterbot)
# IP: 91.98.229.173 (Nuremberg, CPX32)
#
# Bootstrap process:
#   1. Boot Hetzner server into rescue mode (Linux 64-bit)
#   2. Run: make hetzner/bootstrap0 NIXADDR=<ip> NIXNAME=peterbot
#   3. After reboot: make hetzner/bootstrap NIXADDR=<ip> NIXNAME=peterbot
#   4. Connect: ssh peterbot (uses ~/.ssh config)

{
  imports = [
    ./hardware/peterbot.nix
    ../modules/cachix.nix
    ../modules/secrets.nix
  ...
  services.nixosAutoUpdate = {
    enable = true;
    flake = "github:javdl/nixos-config#peterbot";
    dates = "04:00";
  ...
  users.users.peter.shell = lib.mkForce pkgs.zsh;
