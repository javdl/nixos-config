...
# Hetzner dedicated GitHub Actions runner for fuww org (github-runner-06)
# EX63 dedicated server (Intel i9-9900K, 64GB RAM, 2x 1TB NVMe).
# IP: 136.243.104.36 / 2a01:4f8:171:d10::2
#
...
#   1. Confirm server is reachable: ssh root@136.243.104.36
...
#   3. Run: make hetzner/provision NIXADDR=136.243.104.36 NIXNAME=github-runner-06
#   4. SOPS re-key (host age key changes from loom's placeholder to real):
#        ssh-keyscan 136.243.104.36 | grep ed25519 | ssh-to-age
#        # Replace the github-runner-06 anchor in .sops.yaml with the new key
#        sops updatekeys secrets/github-runner-06.yaml
#        # Rebuild so the runner can decrypt the token
#        make hetzner/copy NIXADDR=136.243.104.36 NIXUSER=joost
#        make hetzner/switch NIXADDR=136.243.104.36 NIXNAME=github-runner-06 NIXUSER=joost
#   5. Set up Tailscale: make hetzner/tailscale-auth NIXADDR=136.243.104.36 TAILSCALE_AUTHKEY=tskey-auth-xxx
#   6. Rotate the github-runner-token in SOPS with a fresh org registration token:
#        sops secrets/github-runner-06.yaml
...

{
  ...
  # Hostname
  networking.hostName = "github-runner-06";
  ...
  services.nixosAutoUpdate = {
    enable = true;
    flake = "github:javdl/nixos-config#github-runner-06";
    dates = "04:00";
  ...
  sops.defaultSopsFile = ../secrets/github-runner-06.yaml;
    ...
    lib.listToAttrs (
      lib.genList (
        ...
        lib.nameValuePair "fuww-runner-${toString idx}" {
          ...
          ephemeral = false;
          replace = true;
          name = "github-runner-06-${toString idx}";
