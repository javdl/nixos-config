...
#   2. Run: make hetzner/bootstrap0 NIXADDR=<ip> NIXNAME=rajbot
#   3. After reboot: make hetzner/bootstrap NIXADDR=<ip> NIXNAME=rajbot
#   4. Connect: ssh rajbot (uses ~/.ssh config)

{
  ...
  services.nixosAutoUpdate = {
    enable = true;
    flake = "github:javdl/nixos-config#rajbot";
    dates = "04:00";
  ...
  users.users.rajesh.shell = lib.mkForce pkgs.zsh;
