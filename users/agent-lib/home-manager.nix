# Shared home-manager profile for agent dev boxes.
#
# Parameterized by git identity so every agent user (jay, and any future agent)
# reuses one definition instead of copy-pasting ~600 lines. A per-agent file
# (users/<agent>/home-manager-server.nix) is just:
#
#   import ../agent-lib/home-manager.nix {
#     gitName    = "Agent Jay";
#     gitEmail   = "jay@fashionunited.com";
#     githubUser = "agent-jay";
#   }
#
# The runtime that rondo needs (elixir/erlang/node/claude-code/gh) is managed by
# the agent via mise — mise itself is on every host through mksystem's
# sharedModules. Claude Code is authenticated by the agent with `claude` /login
# (OAuth), and LINEAR_API_KEY is exported by the agent; neither lives in the repo.
...
in
{
  ...
  # Zellij layouts for fuww projects
